...
@ApiResponses({
    ...
    @ApiResponse(responseCode="409",description="REVISION_CONFLICT, PROJECT_ARCHIVED or IDEMPOTENCY_KEY_REUSED",content=@Content(schema=@Schema(implementation=ApiError.class))),
...
public class JournalController {
    ...
    public ResponseEntity<JournalResponse> get(Authentication auth,@PathVariable String id) {
    ...
    @Operation(summary="Update a Journal",description="Omitted fields stay unchanged; revision is required. Same-value updates advance revision. Retaining an archived Project is allowed; reassignment requires an active owned target.",
        parameters=@Parameter(name="X-CSRF-Token",in=ParameterIn.HEADER,required=true,schema=@Schema(type="string")))
    public ResponseEntity<JournalResponse> update(Authentication auth,@PathVariable String id,@Valid @RequestBody UpdateJournalRequest request) {
    ...
    @Operation(summary="Permanently delete an owned Journal",description="Requires the current revision. Returns deletedId. Subsequent reads and mutations return 404.",
        parameters=@Parameter(name="X-CSRF-Token",in=ParameterIn.HEADER,required=true,schema=@Schema(type="string")))
    public ResponseEntity<DeleteJournalResponse> delete(Authentication auth,@PathVariable String id,
