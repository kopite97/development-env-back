...
@RestController
@RequestMapping(value="/api/v2/dashboards/home",produces="application/json")
...
public class HomeDashboardController {
    ...
    @Operation(summary="Read Home Dashboard",description="No query parameters. Unsaved returns revision 0 and default configuration without persistence writes. Saved empty array stays empty. Missing Category selections retain UUID and return selectionState=missingCategory; no repair or fallback. Broken Project references return 404.")
    ...
    @Operation(summary="Save Home Dashboard",description="Full atomic ordered replacement. First PUT revision 0 becomes 1; concurrent first saves have one success and one 409. Existing revision must match; unchanged saves increment. Owned archived Projects permitted. Selection kinds: all, uncategorized, project, category. Existing identical missing Category selection under the same widget ID can be retained; newly missing references return 404. selectionState is output only. No business-resource mutation or default numeric limits.",
    ...
    private ResponseEntity<HomeDashboardResponse> ok(HomeDashboardSnapshot s){return com.kopite.devspace.global.response.WorkspaceResponses.ok(HomeDashboardResponse.from(s),s.dataRevision());}
}
