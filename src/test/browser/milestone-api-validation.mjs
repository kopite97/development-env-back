...
const output = process.argv[2] || 'build/reports/milestone-api/browser';
...
const expectedOperations = [
  { method: 'GET', path: '/api/v1/milestones', label: 'List owned Milestones' },
  { method: 'POST', path: '/api/v1/milestones', label: 'Create an owned Milestone' },
  { method: 'GET', path: '/api/v1/milestones/{id}', label: 'Read an owned Milestone' },
  { method: 'PATCH', path: '/api/v1/milestones/{id}', label: 'Update a Milestone' },
  { method: 'DELETE', path: '/api/v1/milestones/{id}', label: 'Permanently delete an owned Milestone' },
];
const expectedSchemas = ['MilestoneResponse', 'CreateMilestoneRequest', 'UpdateMilestoneRequest', 'MilestoneListResponse', 'DeleteMilestoneResponse'];
...
try {
  ...
  if (rendered.operations.length !== expectedOperations.length || rendered.schemas.length !== expectedSchemas.length)
    throw new Error('Missing visible Milestone operations or schema names: ' + JSON.stringify(rendered));
  const contract = await evaluate(`(async () => {
    ...
    for (const [path, methods] of Object.entries(apiDocument.paths)) {
      if (!path.startsWith('/api/v1/milestones')) continue;
  ...
  const evidence = {result:'MILESTONE_API_SWAGGER_DOCUMENTATION_PASS', ...contract, ...rendered};
