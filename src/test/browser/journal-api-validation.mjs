...
const output = process.env.JOURNAL_BROWSER_OUTPUT || 'build/reports/journal-api/browser';
...
const expectedOperations = [
  { method: 'GET', path: '/api/v1/journals', label: 'List owned Journals' },
  { method: 'POST', path: '/api/v1/journals', label: 'Create an owned Journal' },
  { method: 'GET', path: '/api/v1/journals/{id}', label: 'Read an owned Journal' },
  { method: 'PATCH', path: '/api/v1/journals/{id}', label: 'Update a Journal' },
  { method: 'DELETE', path: '/api/v1/journals/{id}', label: 'Permanently delete an owned Journal' },
];
const expectedSchemas = ['JournalResponse', 'CreateJournalRequest', 'UpdateJournalRequest', 'JournalListResponse', 'DeleteJournalResponse'];
...
try {
  ...
  if (rendered.operations.length !== expectedOperations.length || rendered.schemas.length !== expectedSchemas.length)
    throw new Error('Missing visible Journal operations or schema names: ' + JSON.stringify(rendered));
  const contract = await evaluate(`(async () => {
    ...
    for (const [path, methods] of Object.entries(apiDocument.paths)) {
      if (!path.startsWith('/api/v1/journals')) continue;
  ...
  const evidence = {result:'JOURNAL_API_SWAGGER_DOCUMENTATION_PASS', ...contract, ...rendered};
