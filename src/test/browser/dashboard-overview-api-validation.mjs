...
const output = process.argv[2] || 'build/reports/dashboard-overview-api/browser';
...
const expectedOperations = [
  { method: 'GET', path: '/api/v1/overview', label: 'Read workspace Overview' },
  { method: 'GET', path: '/api/v1/dashboards/home', label: 'Read Home Dashboard' },
  { method: 'PUT', path: '/api/v1/dashboards/home', label: 'Save Home Dashboard' },
];
const expectedSchemas = ['HomeDashboardResponse', 'SaveHomeDashboardRequest', 'DashboardWidgetRequest', 'DashboardWidgetResponse', 'DashboardDataWidget', 'DashboardUtilityWidget', 'OverviewResponse', 'OverviewProjectCounts', 'OverviewProjectScopeCounts', 'OverviewProjectsByScope', 'OverviewTaskCounts'];
...
try {
  ...
  if (rendered.operations.length !== expectedOperations.length || rendered.schemas.length !== expectedSchemas.length)
    throw new Error('Missing visible Dashboard/Overview operations or schema names: ' + JSON.stringify(rendered));
  const contract = await evaluate(`(async () => {
    ...
    for (const [path, methods] of Object.entries(apiDocument.paths)) {
      if (path !== '/api/v1/dashboards/home') continue;
  ...
  const evidence = {result:'DASHBOARD_OVERVIEW_SWAGGER_DOCUMENTATION_PASS', ...contract, ...rendered};
