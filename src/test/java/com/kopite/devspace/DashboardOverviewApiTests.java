...
class DashboardOverviewApiTests {
    static final String HOME="/api/v2/dashboards/home",OVERVIEW="/api/v2/overview";
    ...
    String body(long revision,Object widgets){return json.writeValueAsString(Map.of("schemaVersion",2,"revision",revision,"widgets",widgets));}
    ...
    @Test void virtualDefaultSavedEmptyNormalizedResponseAndStrictFields() throws Exception {
        var o=owner();var d=read(o,HOME);assertEquals(Set.of("id","schemaVersion","revision","widgets"),keys(d));assertEquals("home",d.get("id").asString());assertEquals(2,d.get("schemaVersion").asInt());assertEquals(0,d.get("revision").asInt());assertEquals(6,d.get("widgets").size());
        ...
        UUID archived=project(o,"server",true);var w=widget("overview");w.put("id"," widget ");w.put("selection",Map.of("kind","project","projectId",archived.toString()));w.put("limit",20);
        var saved=response(putHome(o,body(1,List.of(w))));var item=saved.get("widgets").get(0);
        assertEquals(Set.of("id","type","title","selection","selectionState","size","limit"),keys(item));assertEquals("widget",item.get("id").asString());assertEquals("Title",item.get("title").asString());assertEquals("project",item.path("selection").path("kind").asString());assertEquals(20,item.get("limit").asInt());
    ...
    @Test void allWidgetCombinationsAndOmittedDefaults() throws Exception {
        ...
        UUID category=category(o,"Reference");
        for(String type:List.of("overview","board","deploy","links","journal","milestone"))for(String size:List.of("small","medium","wide"))
            for(String kind:List.of("all","uncategorized","project","category")) {
                var w=widget(type);w.put("size",size);var selection=new LinkedHashMap<String,Object>();selection.put("kind",kind);
                if(kind.equals("project"))selection.put("projectId",p.toString());if(kind.equals("category"))selection.put("categoryId",category.toString());w.put("selection",selection);
                if(type.equals("deploy")&&!kind.equals("all")){mvc.perform(putHome(o,body(revision,List.of(w)))).andExpect(status().isBadRequest());continue;}
                var item=response(putHome(o,body(revision++,List.of(w)))).path("widgets").get(0);
                assertEquals(kind,item.path("selection").path("kind").asString());assertEquals("valid",item.path("selectionState").asString());assertEquals(size,item.path("size").asString());assertFalse(item.has("limit"));assertFalse(item.has("projectId"));assertFalse(item.has("scope"));
            }
        for(String type:List.of("overview","board","journal","milestone")) {
            var w=widget(type);w.put("selection",Map.of("kind","project","projectId",p.toString()));w.put("limit",1);
            assertEquals("project",response(putHome(o,body(revision++,List.of(w)))).path("widgets").get(0).path("selection").path("kind").asString());
        }
    ...
    @Test void malformedAndConditionalWritesNeverCreateState() throws Exception {
        ...
        bad.addAll(List.of("[]","null","{}","{","{\"schemaVersion\":2,\"revision\":0}","{\"schemaVersion\":2,\"revision\":0,\"widgets\":null}","{\"schemaVersion\":2,\"revision\":0,\"widgets\":[null]}","{\"schemaVersion\":2,\"revision\":0,\"revision\":0,\"widgets\":[]}"));
        for(String r:List.of("-1","9007199254740992","999999999999999999999999","0.0","\"0\"","null"))
            bad.add("{\"schemaVersion\":2,\"revision\":"+r+",\"widgets\":[]}");
        ...
        for(String extra:List.of("id","workspaceId","ownerUserId","userId","createdAt","position"))
            bad.add("{\"schemaVersion\":2,\"revision\":0,\"widgets\":[],\""+extra+"\":\"x\"}");
        for(String field:List.of("id","type","title","selection","size","projectId","limit")) {
        ...
        bad.add("{\"schemaVersion\":2,\"revision\":0,\"widgets\":[{\"id\":\"a\",\"id\":\"b\",\"type\":\"board\",\"title\":\"T\",\"scope\":\"all\",\"size\":\"wide\"}]}");
        for(String b:bad) mvc.perform(putHome(o,b)).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
        for(String version:List.of("0","1","3","-1","999999999999999999999999"))mvc.perform(putHome(o,"{\"schemaVersion\":"+version+",\"revision\":0,\"widgets\":[]}")).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("UNSUPPORTED_SCHEMA_VERSION"));
    ...
    @Test void strictQueriesAndAuthorization() throws Exception {
        ...
        for(UUID id:List.of(foreign,UUID.randomUUID())) {
            ...
            var w=widget("journal");w.put("selection",Map.of("kind","project","projectId",id.toString()));mvc.perform(putHome(o,body(99,List.of(w)))).andExpect(status().isNotFound());
        }
